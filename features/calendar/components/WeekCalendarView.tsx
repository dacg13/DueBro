...
export function WeekCalendarView({
  ...
  return (
    <div className="rounded-2xl bg-graphite-600/18 backdrop-blur-[20px] border border-white/8 overflow-hidden shadow-2xl">
      {/* 7 Columns Grid */}
      <div className="grid grid-cols-1 md:grid-cols-7 divide-y md:divide-y-0 md:divide-x divide-white/6 items-stretch">
        {weekDays.map((day) => {
          ...
          return (
            <div
              key={dateStr}
              className={cn(
                "flex flex-col min-h-[140px] p-2.5 transition-colors relative group/col",
                isDayToday ? "bg-white/[0.03]" : "hover:bg-white/[0.015]"
              )}
            >
              {/* Clean Column Header */}
              <div className="pb-2 mb-2 border-b border-white/6 flex items-center justify-between">
              ...
              <div className="space-y-1.5 flex-1">
                {dayDeadlines.map((dl) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        "p-2 rounded-xl bg-void-900/70 hover:bg-void-850 border border-white/8 hover:border-white/20 transition-all duration-150 cursor-pointer group/card relative overflow-hidden flex flex-col justify-between gap-1.5",
                ...
                {/* Subtle Hover Action on Empty Column */}
                {dayDeadlines.length === 0 && (
                  <button
                    type="button"
                    onClick={() => onAddDeadlineForDate(dateStr)}
                    className="w-full py-3 rounded-lg border border-dashed border-white/6 hover:border-white/16 hover:bg-white/[0.02] text-mist-200/30 hover:text-mist-100 text-[10px] font-medium flex items-center justify-center gap-1 transition-all opacity-0 group-hover/col:opacity-100 cursor-pointer"
                  >
